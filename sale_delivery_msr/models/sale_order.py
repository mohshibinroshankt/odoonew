...
class AccountMove(models.Model):
    ...
    def onchange_invoice_origin(self):
        if self.invoice_origin:
            ...
            if sale_order:
                self.delivery_charges = sale_order.delivery_charges
    ...
    def create(self, vals):
        ...
        if vals.get('invoice_origin'):
            ...
            if sale_order:
                account_move.delivery_charges = sale_order.delivery_charges
        return account_move

    # else:
    #     # If delivery charges are 0, remove any existing delivery charges order line
    #     delivery_charges_product = self.env['product.product'].search([('name', '=', 'Delivery Charges')],
    #                                                                   limit=1)
    #     order_line = self.order_line.filtered(lambda l: l.product_id == delivery_charges_product)
    #     if order_line:
